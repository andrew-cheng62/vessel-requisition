...
export default function EditRequisition() {
  ...
  // FIX: removed broken early-return inside useEffect (hooks can't return JSX)
  // Non-draft guard is now rendered below
  ...
  if (!isEditable) {
    return (
      <PageContainer title={`Edit Requisition #${id}`}>
        <p className="text-red-600">Editing is not allowed for status: <strong>{status}</strong></p>
    ...
    <PageContainer title={`Edit Requisition #${id}`}>
      <FormLayout onSubmit={handleSave}>
        ...
        <div className="mt-8 flex justify-end gap-3">
          ...
          <Button type="button" variant="delete" onClick={() => navigate(-1)}>Cancel</Button>
        </div>
